...
bool lerUmaLinhaDoArquivoCSV(ifstream& arqEnt, FichaHosp& umPaciente) {
    if (!(arqEnt >> umPaciente.identificador)) {
        return false;
    }
    ...
    arqEnt.getline(umPaciente.nome, 30, ',');
    if (!arqEnt) {
        return false;
    }
    ...
    arqEnt.getline(umPaciente.patologia, 30, ',');
    if (!arqEnt) {
        return false;
    }
...
FichaHosp* lerDadosDoArquivoCSV(ifstream& arqEnt, FichaHosp* Paciente, int& capacidade, int& tamanho){
	FichaHosp umPaciente;
	
	while(lerUmaLinhaDoArquivoCSV(arqEnt, umPaciente)){
		if(tamanho == capacidade){
			aumentarCapacidade(Paciente, capacidade, tamanho);
		}
		Paciente[tamanho] = umPaciente; 
		tamanho++;
	}
	return Paciente;
}

void imprimirListaCSV(FichaHosp Paciente[], int tamanho) {
...
void imprimirListaBinario(FichaHosp Paciente[], int tamanho) {
    for (int i = 0; i < tamanho - 1; i++) {
        cout << Paciente[i].identificador << " " << Paciente[i].nome << " "
             << Paciente[i].genero << " " << Paciente[i].peso << "kg "
             << Paciente[i].altura << "m " << Paciente[i].patologia << endl
...
void imprimirLista(FichaHosp Paciente[], int tamanho, bool imprimirDoBinario) {
    if (imprimirDoBinario) {
        imprimirListaBinario(Paciente, tamanho);
    } else {
        imprimirListaCSV(Paciente, tamanho);
    }
}
...
void ordenarLista(int tamanho, FichaHosp *Paciente, int capacidade){
...
void menuPrincipal(int capacidade, int tamanho, FichaHosp *Paciente) // MENU DE ESCOLHA QUE É APRESENTADO AS FUNCIONALIDADES DO CODIGO, CHAMA A FUNÇAO "menu1ou0" APOS O 'CASE' FINALIZAR
{
    ...
    {
    case 1:
        bool imprimirDoBinario;
        cout << "Escolha a fonte de impressao:" << endl
             << "[1] Arquivo Binario" << endl
             << "[2] Arquivo CSV" << endl;
        cin >> imprimirDoBinario;
        imprimirLista(Paciente, tamanho, imprimirDoBinario);
    ...
    case 7:
        ...
        break;
    case 8:
        ordenarLista(tamanho, Paciente, capacidade);
        menu1ou0(capacidade, tamanho, Paciente);
        break;
    default:
        break;
